...
#define SENS_TYP_OS_5D50        0X5D50  // BTHR918
class Sensor {
...
//	double  train;
	double	pressure;
	char    confortLabel[16];
	char    confort;
	char    forecastLabel[16];
	char    forecast;
	char    raw [100];
...
//	bool	haveTrain;			// true when hum capacity decoded
	...
	bool    haveForecast;          // true when prevision capacity decoded
...
protected:
    ...
    bool writeElement(int fileDescription, char * Element, char * stringToWrite);
    bool writeElement(int fileDescription, char * Element, int intToWrite,char * format);
    bool writeElement(int fileDescription, char * Element, double doubleToWrite,char * format);
    bool writeElement(int fileDescription, char * Element, time_t timeToWrite);
...
public:
	...
	bool hasForecast();           // return true if valid && hasPrevision 
	...
	bool getBattery();
	char * getConfortLabel();
	char * getForecastLabel();          // return Prevision information
	time_t getCreationTime();	// return object creation time
	bool writeXML(char * s );
